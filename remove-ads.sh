#!/usr/bin/env bash
# สคริปต์ลบโค้ดที่เกี่ยวกับโฆษณา (Google AdSense) ออกจากโปรเจกต์ Font Tai ทั้งหมด
# วิธีใช้: รันจาก root ของโปรเจกต์ (โฟลเดอร์ที่มี package.json) ด้วยคำสั่ง: bash remove-ads.sh
set -e

echo "== ลบ public/ads.txt =="
rm -f public/ads.txt

echo "== เขียนไฟล์ index.html ใหม่ =="
mkdir -p "$(dirname 'index.html')"
cat > 'index.html' << 'FONTTAI_EOF_INDEX_HTML'
<!doctype html>
<html lang="th">
  <head>
    <meta charset="UTF-8" />
    <title>Font Tai ၾွၼ်ႉတႆး - ฟอนต์ไต ฟอนต์ไทใหญ่ Shan Font Preview & Download</title>
    <meta name="description" content="Font Tai ၾွၼ်ႉတႆး ၾွၼ်ႉတႆးယူႇၼီႇၶူတ်ႉ ၸၼ်ဢဝ်ၾွၼ်ႉတႆးႁၼ်သႃႇတႆၢႇၵမ်းသိုဝ်ႈ ၽင်ၾွၼ်ႉတႆး ၾွၼ်ႉတႆးမီးတၢင်းၼမ်ၶႃႈ แหล่งรวมฟอนต์ไต ฟอนต์ไทใหญ่ ดาวน์โหลดฟอนต์ ฝังฟอนต์ไตในเว็บไซต์ มีโค้ดสำหรับฝังในเว็บไซต์ เพื่อให้สามารถอ่านภาษาไตได้ง่าย" />
    ...
    <link
      ...
      rel="stylesheet"
    />
    <link rel="alternate" hreflang="th" href="https://fonttai.com/" />
<link rel="alternate" hreflang="en" href="https://fonttai.com/en/" />
<link rel="alternate" hreflang="x-default" href="https://fonttai.com/" />
  </head>
...
FONTTAI_EOF_INDEX_HTML

echo "== เขียนไฟล์ src/utils/consentScripts.ts ใหม่ =="
mkdir -p "$(dirname 'src/utils/consentScripts.ts')"
cat > 'src/utils/consentScripts.ts' << 'FONTTAI_EOF_SRC_UTILS_CONSENTSCRIPTS_TS'
import {
...
FONTTAI_EOF_SRC_UTILS_CONSENTSCRIPTS_TS

echo "== เขียนไฟล์ src/utils/cookieConsent.ts ใหม่ =="
mkdir -p "$(dirname 'src/utils/cookieConsent.ts')"
cat > 'src/utils/cookieConsent.ts' << 'FONTTAI_EOF_SRC_UTILS_COOKIECONSENT_TS'
...
FONTTAI_EOF_SRC_UTILS_COOKIECONSENT_TS

echo "== เขียนไฟล์ src/components/CookieSettingsModal.tsx ใหม่ =="
mkdir -p "$(dirname 'src/components/CookieSettingsModal.tsx')"
cat > 'src/components/CookieSettingsModal.tsx' << 'FONTTAI_EOF_SRC_COMPONENTS_COOKIESETTINGSMODAL_TSX'
...
FONTTAI_EOF_SRC_COMPONENTS_COOKIESETTINGSMODAL_TSX

echo "== เขียนไฟล์ src/components/CookieBanner.tsx ใหม่ =="
mkdir -p "$(dirname 'src/components/CookieBanner.tsx')"
cat > 'src/components/CookieBanner.tsx' << 'FONTTAI_EOF_SRC_COMPONENTS_COOKIEBANNER_TSX'
...
FONTTAI_EOF_SRC_COMPONENTS_COOKIEBANNER_TSX

echo "== เขียนไฟล์ src/App.tsx ใหม่ =="
mkdir -p "$(dirname 'src/App.tsx')"
cat > 'src/App.tsx' << 'FONTTAI_EOF_SRC_APP_TSX'
import { useEffect, useMemo, useState } from "react";
...
FONTTAI_EOF_SRC_APP_TSX

echo "== เสร็จสิ้น: ลบโค้ดที่เกี่ยวกับโฆษณาทั้งหมดแล้ว =="
echo "ตรวจสอบด้วย: npm install && npm run build"
